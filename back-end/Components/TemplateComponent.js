...
exports.updateTemplateImage = async (req, res) => {
  ...
  try {
    ...
    if (!TemplateImage) {
      return res
        .status(400)
        .json({ message: "Template Image URL is required" });
    }
    ...
    res
      .status(200)
      .json({ message: "Template image updated successfully", template });
  } catch (err) {
...
exports.GetProposalByLimit = async (req, res) => {
  try {
    ...
    const ProposalsData = await ProposalModel.find({ Users: userObjectId })
      .skip(skip)
      .limit(limit)
      .sort({ createdAt: -1 })
      .populate("workspaces");
